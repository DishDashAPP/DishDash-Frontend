...
const RestaurantMenu: FC = () => {
    ...
    const handleCategoryChange = (category: TChip) => {
        ...
        if (category.id === 0) {
            ...
            setFilteredItems(menuItems.filter((item) => item.category_id === category.id))
        }
    }
        ...
        if (updatedFoodsRes.isSuccess) {
            ...
            setFilteredItems(
                selectedCategory?.id === 0
                    ...
                    : updatedFoodsRes.data.filter((item) => item.category_id === selectedCategory?.id)
            )
        }
    }
    ...
    return (
        <div className="flex flex-col items-center w-full">
            ...
            {loading ? (
                <div className="my-8 text-base animate-pulse">در حال دریافت اطلاعات...</div>
            ) : (
                <>
                    <Chips
                        chips={categories}
                        ...
                        className="mb-6"
                    />
                    <MenuItems menuItems={filteredItems} onDelete={handleDelete} onUpdate={handleUpdate} />
                </>
            )}
            <button
            ...
            <BottomSheet isOpen={isBottomSheetOpen} onClose={() => setBottomSheetOpen(false)}>
                <NewFoodItem mode="add" onUpdate={handleUpdate} onClose={() => setBottomSheetOpen(false)} />
            </BottomSheet>
        </div>
    )
}
