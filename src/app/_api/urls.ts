...
export const RESTAURANT_OWNER_ORDER = {
    UPDATE_ORDER_STATUS: (orderId: string, status: string) => `/order/restaurantOwner/status?orderId=${orderId}&status=${status}`,
...
export const DELIVERY_PERSON_ORDER = {
    UPDATE_ORDER_STATUS: (orderId: string, status: string) => `/order/deliveryPerson/status?orderId=${orderId}&status=${status}`,
