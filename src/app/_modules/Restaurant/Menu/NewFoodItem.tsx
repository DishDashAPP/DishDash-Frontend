...
import {
  ...
  editFoodReq,
  getAllCategoriesReq,
} from "@api/services/restaurantService"; // Adjust the import path accordingly
...
const NewFoodItem: FC<NewFoodItemProps> = ({
  mode,
  foodItem,
  onClose,
  onUpdate,
      ...
      if (categoriesRes.isSuccess) {
        ...
        if (mode === "edit" && foodItem) {
          ...
          setSelectedCategory({
            ...
            name:
              categoriesRes.data.find((cat) => cat.id === foodItem.category_id)
                ?.name || "",
          });
