...
import { CustomerOrderType, OrderItem } from '@utils/types'
...
type ShoppingCartCardProps = {
    shoppingCart: CustomerOrderType | any
    isHistory?: boolean
}
        ...
        <>
            <Link href={SHOPPING_CARTS + '/' + shoppingCart.id}>
                <div className={'px-8 py-4 mt-7 mb-6 border-[1px] border-gray-card_border rounded-lg'}>
                    <div className={'flex justify-between'}>
                        ...
                        {isHistory && (
                            <div className={'flex'} onClick={handleComment}>
                                <Image src={COMMENT} width={20} height={20} alt={'comment'} className={'ml-2'} />
                                <span className={'text-xs font-normal text-gray-primary underline underline-offset-4'}>
                                    ثبت امتیاز و نظر
                                </span>
                            </div>
                        )}
                    ...
                    <div className={'mt-5'}>
                        {shoppingCart.order_items.map((food: OrderItem) => (
                            <div className={'flex mt-1'} key={food.food_id}>
                    ...
                    <div className={'flex justify-between mt-5 gap-x-4'}>
                        {isHistory ? (
                            <Button label={'مشاهده‌ی فاکتور'} variant={'secondary'} className={'w-full'} />
                        ) : (
                            <>
